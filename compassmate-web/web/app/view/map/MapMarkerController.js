...
Ext.define('Compassmate.view.map.MapMarkerController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.mapMarker',

    requires: [
        'Compassmate.model.Position',
        'Compassmate.model.Device',
        'Compassmate.DeviceImages'
    ],
    ...
    init: function () {
        ...
        this.liveRoutes = {};
        this.liveRouteLength = Compassmate.app.getAttributePreference('web.liveRouteLength', 10);
        this.selectZoom = Compassmate.app.getAttributePreference('web.selectZoom', 0);
    },
    ...
    getAreaStyle: function (label, color) {
        ...
        if (color) {
            ...
            fillColor = ol.color.asArray(Compassmate.Style.mapGeofenceColor);
            strokeColor = Compassmate.Style.mapGeofenceColor;
        }
        fillColor[3] = Compassmate.Style.mapGeofenceOverlayOpacity;
        styleConfig = {
            ...
            stroke: new ol.style.Stroke({
                color: strokeColor,
                width: Compassmate.Style.mapGeofenceWidth
            })
        };
        if (label) {
            styleConfig.text = new ol.style.Text({
                text: label,
                fill: new ol.style.Fill({
                    color: Compassmate.Style.mapGeofenceTextColor
                }),
                stroke: new ol.style.Stroke({
                    color: Compassmate.Style.mapTextStrokeColor,
                    width: Compassmate.Style.mapTextStrokeWidth
                }),
                font: Compassmate.Style.mapTextFont
            });
        }
    ...
    getDeviceColor: function (device) {
        switch (device.get('status')) {
            case 'online':
                return Compassmate.Style.mapColorOnline;
            case 'offline':
                return Compassmate.Style.mapColorOffline;
            default:
                return Compassmate.Style.mapColorUnknown;
        }
    },
    ...
    updateAccuracy: function (position, device) {
        ...
        if (position.get('accuracy')) {
            ...
            if (feature) {
                ...
                feature.setStyle(this.getAreaStyle(null, Compassmate.Style.mapAccuracyColor));
    ...
    clearReport: function () {
        ...
        if (this.selectedMarker && !this.selectedMarker.get('event') &&
                this.selectedMarker.get('record') instanceof Compassmate.model.Position) {
    ...
    getRouteStyle: function (deviceId) {
        return new ol.style.Style({
            stroke: new ol.style.Stroke({
                color: Compassmate.app.getReportColor(deviceId),
                width: Compassmate.Style.mapRouteWidth
            })
        });
    },

    getMarkerStyle: function (zoom, color, angle, category) {
        var image = Compassmate.DeviceImages.getImageIcon(color, zoom, angle, category);
        return new ol.style.Style({
            image: image,
            text: new ol.style.Text({
                textBaseline: 'bottom',
                fill: new ol.style.Fill({
                    color: Compassmate.Style.mapTextColor
                }),
                stroke: new ol.style.Stroke({
                    color: Compassmate.Style.mapTextStrokeColor,
                    width: Compassmate.Style.mapTextStrokeWidth
                }),
                offsetY: -image.getSize()[1] / 2 - Compassmate.Style.mapTextOffset,
                font: Compassmate.Style.mapTextFont
            })
        });
    },
    ...
    getReportMarker: function (deviceId, angle) {
        return this.getMarkerStyle(false, Compassmate.app.getReportColor(deviceId), angle, 'arrow');
    },

    resizeMarker: function (style, zoom) {
        var image, text;
        image = Compassmate.DeviceImages.getImageIcon(
            style.getImage().fill, zoom, style.getImage().angle, style.getImage().category);
        text = style.getText();
        text.setOffsetY(-image.getSize()[1] / 2 - Compassmate.Style.mapTextOffset);
        style.setText(text);
        style.setImage(image);
    },

    rotateMarker: function (style, angle) {
        style.setImage(Compassmate.DeviceImages.getImageIcon(
    ...
    selectFeature: function (feature) {
        ...
        if (record) {
            if (record instanceof Compassmate.model.Device) {
