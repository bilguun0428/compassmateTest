...
import mn.compassmate.reports.model.BaseReport;
...
public final class ReportUtils {
    ...
    public static double solveEquation(ImmutablePair ip, int x) {
        BigDecimal left = (BigDecimal) ip.left;
        BigDecimal right = (BigDecimal) ip.right;
        BigDecimal ret = left.multiply(new BigDecimal(x)).add(right);
        //return (double) ip.getLeft() * x + (double) ip.getRight();
        return ret.doubleValue();
    }

    public static ImmutablePair<BigDecimal, BigDecimal> predictEquation(ArrayList al) {
        BigDecimal sumx = new BigDecimal(0.0),
                sumy = new BigDecimal(0.0);
        int n;
        n = al.size();
        for (int ii = 0; ii < n; ii++) {
            //sumx  += ii;
            sumx = sumx.add(new BigDecimal(ii));
            //sumy  += (double) al.get(ii);
            sumy = sumy.add(new BigDecimal((double) al.get(ii)));
        }
        BigDecimal xbar = sumx.divide(new BigDecimal(n), 10, RoundingMode.HALF_EVEN);
        BigDecimal ybar = sumy.divide(new BigDecimal(n), 10, RoundingMode.HALF_EVEN);

        // second pass: compute summary statistics
        BigDecimal xxbar = new BigDecimal(0.0), xybar = new BigDecimal(0.0);
        for (int i = 0; i < n; i++) {
            //xxbar += (i - xbar) * (i - xbar);
            xxbar = xxbar.add((new BigDecimal(i))
                    .subtract(xbar)
                    .multiply(
                        (new BigDecimal(i))
                                .subtract(xbar)));
            //xybar += (i - xbar) * ((double) al.get(i) - ybar);
            xybar = xybar.add((new BigDecimal(i))
                    .subtract(xbar)
                    .multiply((new BigDecimal((double) al.get(i)))
                            .subtract(ybar)));
        }
        BigDecimal beta1 = xybar.divide(xxbar, 10, RoundingMode.HALF_EVEN);
        BigDecimal beta0 = ybar.subtract(beta1.multiply(xbar));

        // print results
        //System.out.println("y   = " + beta1 + " * x + " + beta0);
        ImmutablePair<BigDecimal, BigDecimal> ret = ImmutablePair.of(beta1, beta0);
        return ret;
    }
    ...
    public static String getSpeedUnit(long userId) {
        return (String) Context.getPermissionsManager().lookupPreference(userId, "speedUnit", "km");
    }
    ...
    public static BigDecimal calculateSpentFuel(Position firstPosition, Position lastPosition) {

        if (firstPosition.getAttributes().get(Position.KEY_FUEL_LEVEL) != null
                && lastPosition.getAttributes().get(Position.KEY_FUEL_LEVEL) != null) {

            return new BigDecimal(firstPosition.getDouble(Position.KEY_FUEL_LEVEL)
                    - lastPosition.getDouble(Position.KEY_FUEL_LEVEL));
        }
        return new BigDecimal(0.0);
    }

    public static double calculateChargedFuel(Position firstPosition, Position lastPosition) {

        if (firstPosition.getAttributes().get(Position.KEY_FUEL_LEVEL) != null
                && lastPosition.getAttributes().get(Position.KEY_FUEL_LEVEL) != null) {

            BigDecimal value = new BigDecimal(lastPosition.getDouble(Position.KEY_FUEL_LEVEL)
                    - firstPosition.getDouble(Position.KEY_FUEL_LEVEL));
            return value.setScale(1, RoundingMode.HALF_EVEN).doubleValue();
        }
        return 0;
    }
